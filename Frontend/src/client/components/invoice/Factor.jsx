...
function Factor() {
    ...
    return (
        loading || !data ? (
            <div className="w-full h-screen mt-40 max-xs:mt-16">
                <Spinner />
                <div className="text-center mt-10 max-xs:mt-0 max-xs:text-sm">در حال ساخت فاکتور شما...</div>
            </div>
        ) : (
            error ? (
                <Error errorMesage={error} />
            ) : (
                <div className="w-2/5 max-xs:w-full max-xs:px-4 h-auto mx-auto">
                    <div className="mt-28 max-xs:mt-16 mb-20 bg-gray-50" style={{ boxShadow: "0px 0px 20px 0px #0000001a" }}>
                        <div ref={factorRef} className="">
                            ...
                            <div className="p-6 space-y-7 max-xs:space-y-5 text-sm max-xs:text-xs">
                                ...
                                <div className=" text-zinc-500">
                                    ...
                                    <ul className="text-xs mt-4 px-4 bg-zinc-100 text-zinc-500 divide-y">
                                        {data.data.items.map((ticket) => (
                                            <li className="flex justify-between py-5 max-xs:py-4" key={ticket._id}>
                                ...
                                <div className="flex gap-x-2 text-gray-500 items-center my-4">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" strokeWidth="1.5" stroke="#ffae00" className="size-6 align-middle max-xs:hidden">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
                                    </svg>
                                    <p className="text-sm max-xs:bg-yellow-100 max-xs:p-2 max-xs:text-xs max-xs:w-full max-xs:text-center">توجه: لطفا کد پیگیری زیر را در جایی ذخیره نمایید.</p>
                                </div>
                            </div>

                            <div className="flex items-center max-xs:justify-center py-4 px-6 gap-x-2 text-zinc-500 bg-green-400/20 max-xs:text-xs">
                                <span>کد پیگیری:</span>
                                <span className="text-sm max-xs:text-xs">{trackingCode}</span>
                            </div>
                        </div>

                        <div className="flex justify-between max-xs:text-xs">
